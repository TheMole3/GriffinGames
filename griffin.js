...
var griffin = {
    ...
    getPlayerByTarget: async (targetEmail) => { // Get player by specifying target email
        return new Promise((resolve, reject) => {
            db.GriffinGames.findOne({"target": {$regex : new RegExp(targetEmail, "i")}}, function (error, docs) {
                if (error) reject(error);
                resolve(docs);
            });
        }).catch((error) => console.error(error));
    ...
    reportHit: async (emailPlayer, newHitName, newHitClass) => { // Validate if a hit is correct
        ...
        db.GriffinGames.update({'email': {$regex : new RegExp(emailPlayer, "i")}}, {$push: {log: {name: newHitName, class: newHitClass, success: newTarget?target.target == newTarget.email:false, time: Date.now()}}}, {multi: false}, function (error, docs) {})
    ...
    setNewTargetNoPush: async (emailPlayer, targetEmail) => { // Set a new target without pushing old target to prevTargets
        return new Promise((resolve, reject) => {
            db.GriffinGames.update({'email': {$regex : new RegExp(emailPlayer, "i")}}, {$set: {target: targetEmail}}, {multi: false}, function (error, docs) {
                if (error) reject(error);
                resolve(docs);
            })
        }).catch((error) => console.error(error));
